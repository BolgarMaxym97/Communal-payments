...
$value = isset($value) ? $value : old($name, (isset($object) and $object->{$name}) ? $object->{$name}->format('Y-m-d H:i:s') : '');
?>
<div class="form-group {{ $error ? 'has-error' : null }} {{ isset($additionalClass) ? $additionalClass : null }}">
    ...
    @endif
    {!! Form::input('text', $name, $value, array_merge(['class' => 'form-control dateTimePicker', 'placeholder' => $placeholder], $options)) !!}
    @if($error)
